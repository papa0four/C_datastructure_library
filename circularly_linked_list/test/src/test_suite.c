...
void test_append (pf_t * pass_fail)
{
    ...
    pass_fail->tests = realloc(pass_fail->tests, (pass_fail->tests_ran * sizeof(char *)));
    if (NULL == pass_fail->tests)
    {
        errno = ENOMEM;
        fprintf(stderr, "%s(): could not resize tests array: %s\n",
                        __func__, strerror(errno));
        return;
    }

    pass_fail->pf_flags = realloc(pass_fail->pf_flags, (pass_fail->tests_ran * sizeof(ssize_t)));
    if (NULL == pass_fail->pf_flags)
    {
        errno = ENOMEM;
        fprintf(stderr, "%s(): could not resize flags array: %s\n",
                        __func__, strerror(errno));
        return;
    }
...
void test_insert_after_at_tail (pf_t * pass_fail)
{
    ...
    char * test_name    = "test_insert_after_tail\0";
    size_t fname_sz     = get_namelen(test_name);
    pass_fail->tests_ran++;
    ...
    int * new = gen_random();
    test_verbose_after_at_tail(test_name, cll, new, cll->tail);
    node_t * old_tail = cll->tail;
    insert_after(cll, cll->tail, new, INT_T);

    node_t * find = find_node(cll, new);
    if ((NULL == find)
        || (6 != cll->size)
        || (*new != *(int *)find->data)
        || (-1 == cll->compare_func(old_tail->next->data, find->data)))
    {
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 0;
        pass_fail->num_failed++;
        destroy_list(cll);
            ...
            && (6 ==  cll->size)
            && (0 == cll->compare_func(old_tail->next->data, find->data)))
    {
        test_verbose_after_at_tail(test_name, cll, new, find);
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 1;
        pass_fail->num_passed++;
    }
    else 
...
void test_find_by_index (pf_t * pass_fail)
{
    ...
    test_verbose_by_index(test_name, cll, NULL, index);
    node_t * check = find_node(cll, (int *)find->data);
    if ((NULL == check)
        || (NULL == check)
        || (5 != cll->size)
        || (-1 == cll->compare_func(prev->next->data, check->data)))
    {
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 0;
        pass_fail->num_failed++;
        destroy_list(cll);
            ...
            && (0 == cll->compare_func(find->data, check->data))
            && (0 == cll->compare_func(prev->next->data, check->data)))
    {
        test_verbose_by_index(test_name, cll, find, index);
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 1;
        pass_fail->num_passed++;
    }
    else
...
void test_find_by_data (pf_t * pass_fail)
{
    param_check(__FILE__, __LINE__, ARG_1, pass_fail);

    char * test_name    = "test_find_by_data\0";
    size_t fname_sz     = get_namelen(test_name);
    pass_fail->tests_ran++;
    ...
    test_verbose_by_data(test_name, cll, NULL, find->data);
    node_t * check = find_node(cll, (int *)find->data);
    if ((NULL == check)
        || (NULL == check)
        || (5 != cll->size)
        || (-1 == cll->compare_func(prev->next->data, check->data)))
    {
        ...
        destroy_list(cll);
            ...
            && (0 == cll->compare_func(prev->next->data, check->data)))
    {
        test_verbose_by_data(test_name, cll, find, check->data);
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 1;
        pass_fail->num_passed++;
    }
    else
...
void test_delete_node (pf_t * pass_fail)
{
    ...
    char * test_name    = "test_delete_node\0";
    size_t fname_sz     = get_namelen(test_name);
    pass_fail->tests_ran++;
    ...
    size_t    index = gen_index();
    node_t *  find  = find_by_index(cll, index);
    test_verbose_delete_node(test_name, cll, find, (int *)find->data);

    node_t * prev = find_by_index(cll, find->index - 1);
    node_t * next = find->next;
    int    * data = (int *)find->data;

    remove_node(cll, find->data);
    if (5 == cll->size)
    {
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 0;
        pass_fail->num_failed++;
        destroy_list(cll);
            ...
            && (0 == cll->compare_func(prev->next->data, next->data)))
    {
        test_verbose_delete_node(test_name, cll, NULL, data);
        pass_fail->pf_flags[pass_fail->tests_ran - 1] = 1;
        pass_fail->num_passed++;
    }
    else
...
void test_delete_head (pf_t * pass_fail)
{
    ...
    char * test_name    = "test_delete_head\0";
    size_t fname_sz     = get_namelen(test_name);
    pass_fail->tests_ran++;
    ...
    test_verbose_delete_head(test_name, cll);
...
void test_delete_tail (pf_t * pass_fail)
{
    ...
    char * test_name    = "test_delete_tail\0";
    size_t fname_sz     = get_namelen(test_name);
    pass_fail->tests_ran++;
    ...
    test_verbose_delete_tail(test_name, cll);
